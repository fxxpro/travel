...
if(isset($_GET['id'])){
    $id=$_GET['id'];
    $che=mysqli_query($conn,"select * from survey where id ='$id' limit 0,1");
 ...
 if(isset($_POST['submit'])){
     ...
     $che=mysqli_query($conn,"select * from survey where id<>'$id' and name='$name'");
     if(mysqli_fetch_array($che)){
         echo '<script>alert("概况名称已存在");  var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
      ...
      if($upload['name']<>""){
          ...
          if($res['code']!==200){
              ...
              parent.layer.close(index);  var result = window.showModalDialog();if(result){window.location.reload()} </script>';
              exit();
         }
        $filename=$res['filename'];
     }
     $time=time();
     $sql="update survey set name='$name',content='$content',upload='$filename',utime='$time' where id='$id'";
...
<div class="div_left">
    ...
    <form id="form_demo"  action="./editSurvey.php"  enctype="multipart/form-data" method="post" class="form_b">
            ...
            <p class="form_p"><label class="form_label">概况地址：</label><input type="text" name="name" value="<?php echo $strategyInfo['name'];?>" class="form_input " minlength="2"  required ><i class="i_start"></i> </p>
